...
def _shallow_mobilenet(width_mul):
    init_features = int(32 * width_mul)
...
class mobilenet(_fasterRCNN):
    def __init__(self, wm, classes, pretrained=False, class_agnostic=False):
        self.wm = wm
        if wm == 1:
            self.model_path = 'data/pretrained_model/mobilenet-1x.pth.tar'
            self.dout_base_model = 512
            self.dout_top_model = 1024
        elif wm == 0.5:
            self.model_path = 'data/pretrained_model/mobilenet-0.5x.pth.tar'
            self.dout_base_model = 256
            self.dout_top_model = 512
        elif wm == 0.25:
            self.model_path = 'data/pretrained_model/mobilenet-0.25x.pth.tar'
            self.dout_base_model = 128
            self.dout_top_model = 256
        elif wm == 0.125:
            self.model_path = 'data/pretrained_model/mobilenet-0.125x.pth.tar'
            self.dout_base_model = 64
            self.dout_top_model = 128
        else:
    ...
    def _init_modules(self):
        mobilenet = _mobilenet(self.wm)
        if self.pretrained == True:
            print("Loading pretrained weights from %s" %(self.model_path))
            checkpoint = torch.load(self.model_path)
            mobilenet.load_state_dict(checkpoint['state_dict'])

        self.RCNN_base = nn.Sequential(*list(mobilenet.features.children())[:69])
        self.RCNN_top = nn.Sequential(*list(mobilenet.features.children())[69:81])
        ...
        assert (0 <= cfg.MOBILENET.FIXED_LAYERS <= 14)
        if cfg.MOBILENET.FIXED_LAYERS >= 1:
            for _ in range(3):
                for p in self.RCNN_base[_].parameters(): p.requires_grad=False
        if cfg.MOBILENET.FIXED_LAYERS >= 2:
            for _ in range(3, 3 + 6 * (cfg.MOBILENET.FIXED_LAYERS - 1)):
                for p in self.RCNN_base[_].parameters(): p.requires_grad=False
